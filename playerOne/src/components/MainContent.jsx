...
const MainContent = ({
  ...
  const renderTabButtons = (activeTab, setActiveTab, options) => (
    <div className="flex bg-gray-100 p-0.5 rounded-lg text-xs font-medium">
      {options.map(option => (
        <button 
          key={option.value}
          className={`px-2 py-1 rounded-md ${activeTab === option.value 
  ...
  const filteredHabits = habits.filter(habit => {
    if (habitTab === 'strong') return habit.streak >= 10;
    if (habitTab === 'week') {
      // This would normally filter by weekly schedule
        ...
        <div className="flex items-center justify-between">
          ...
          <div className="flex items-center">
            {isCollapsed && (
              <button
                ...
                className="mr-4 p-2 rounded-full border border-gray-200 shadow-sm bg-white transition-colors hover:bg-gray-50"
              >
          ...
          <div className="flex justify-center flex-1">
            <div className="relative w-full max-w-2xl">
              ...
              <input
                ...
                className="pl-10 pr-4 py-2 w-full border border-gray-200 rounded-lg text-sm placeholder-gray-500 focus:outline-none focus:border-purple-500 focus:ring-1 focus:ring-purple-500 bg-white shadow-sm"
                value={searchQuery}
          ...
          <div className="flex items-center">
            <button
              ...
              className="ml-4 flex items-center gap-2 px-4 py-2 rounded-lg bg-gradient-to-r from-purple-600 to-indigo-600 text-white text-sm font-medium hover:shadow-md transition-shadow"
            >
      ...
      <div className="flex-1 pt-2 pb-6 overflow-hidden px-4 sm:px-6 lg:px-8">
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6 h-full">
          ...
          <div className="flex flex-col h-full overflow-hidden">
            {/* Fixed header */}
            <div className="px-4 py-3 border-b border-gray-100 bg-white flex items-center justify-between rounded-t-xl shadow-sm">
              <div className="flex items-center space-x-2">
                <svg xmlns="http://www.w3.org/2000/svg" className="h-5 w-5 text-gray-600" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth="2">
                  <path strokeLinecap="round" strokeLinejoin="round" d="M9 12l2 2 4-4m6 2a9 9 0 11-18 0 9 9 0 0118 0z" />
                </svg>
                <h2 className="text-base font-semibold text-gray-900">Habits</h2>
                <span className="bg-purple-100 text-purple-800 text-xs font-medium px-2.5 py-0.5 rounded-full">{filteredHabits.length}</span>
              </div>
            ...
            <div className="bg-white flex-1 flex flex-col rounded-b-xl shadow-sm overflow-hidden">
              {/* Fixed Add Habit button */}
              <div className="p-4 pb-2">
                <button 
                  onClick={() => handleAddItem("habit")}
                  className="w-full flex items-center justify-center px-4 py-3 border border-dashed border-gray-300 rounded-lg text-sm font-medium text-gray-500 hover:text-gray-700 hover:border-gray-400 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-purple-500 bg-gray-50 hover:bg-gray-100 transition-colors"
                >
          ...
          <div className="flex flex-col h-full overflow-hidden">
            ...
            <div className="px-4 py-3 border-b border-gray-100 bg-white flex items-center justify-between rounded-t-xl shadow-sm">
              <div className="flex items-center space-x-2">
                ...
                <span className="bg-purple-100 text-purple-800 text-xs font-medium px-2.5 py-0.5 rounded-full">{filteredTasks.length}</span>
              </div>
          ...
          <div className="flex flex-col h-full overflow-hidden">
            ...
            <div className="px-4 py-3 border-b border-gray-100 bg-white flex items-center justify-between rounded-t-xl shadow-sm">
              <div className="flex items-center space-x-2">
                ...
                <span className="bg-purple-100 text-purple-800 text-xs font-medium px-2.5 py-0.5 rounded-full">{filteredProjects.length}</span>
              </div>
